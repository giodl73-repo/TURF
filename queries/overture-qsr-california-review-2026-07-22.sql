-- Conservative review layer for California QSR rows.
--
-- Keep primary McDonald's, Burger King, Wendy's, and Taco Bell stores
-- packet-ready and flag incomplete rows for review.
...
COPY (
    WITH raw_rows AS (
        ...
        FROM read_csv_auto('fixtures/stores/overture-qsr-california-2026-07-22.csv', all_varchar = true)
    )
    SELECT
        brand,
        store_id,
...
) TO 'fixtures/stores/overture-qsr-california-review-2026-07-22.csv'
  (HEADER, DELIMITER ',');
