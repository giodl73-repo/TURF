-- Conservative review layer for Pennsylvania / New Jersey national tax-prep rows.
--
-- Keep primary H&R Block, Jackson Hewitt, and Liberty Tax locations
-- packet-ready while excluding incomplete or unrelated candidates.
...
COPY (
    WITH raw_rows AS (
        ...
        FROM read_csv_auto('fixtures/stores/overture-tax-prep-pennsylvania-new-jersey-2026-07-22.csv', all_varchar = true)
    )
    SELECT
        brand,
        store_id,
...
) TO 'fixtures/stores/overture-tax-prep-pennsylvania-new-jersey-review-2026-07-22.csv'
  (HEADER, DELIMITER ',');
