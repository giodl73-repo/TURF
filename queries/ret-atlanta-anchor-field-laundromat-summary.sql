...
COPY (
    ...
    reviewed_rows AS (
        SELECT
            ...
            review_reason
        FROM read_csv_auto(
            'fixtures/civic/osm-laundromat-atlanta-anchor-fields-review-2026-08-13.csv',
            all_varchar = true
        )
    ),
    gates AS (
        SELECT
            ...
            gate_reason
        FROM read_csv_auto(
            'fixtures/civic/osm-laundromat-atlanta-anchor-fields-source-gates-2026-08-13.csv',
            all_varchar = true
        )
    ),
    summarized AS (
        SELECT
            ...
            count(reviewed_rows.facility_name) AS raw_laundromat_rows,
            sum(CASE WHEN reviewed_rows.review_status = 'packet_ready' THEN 1 ELSE 0 END) AS laundromat_rows,
            count(DISTINCT CASE
            ...
            sum(CASE WHEN reviewed_rows.review_reason = 'unnamed_household_service_anchor' THEN 1 ELSE 0 END) AS unnamed_household_service_anchor_rows,
            string_agg(
    ...
    SELECT
        ...
        unnamed_household_service_anchor_rows,
        coalesce(laundromat_names, '') AS laundromat_names,
        CASE
            WHEN gate_status = 'source_gated' THEN 'source_gated'
            WHEN laundromat_rows >= 2 THEN 'observed_dense'
            WHEN laundromat_rows > 0 THEN 'observed'
            ELSE 'checked_absent'
        END AS laundromat_signal,
    ...
    ORDER BY
        CASE field_id
            ...
            ELSE 99
        END
) TO 'reports/ret-atlanta-anchor-field-laundromat-summary.csv'
  (HEADER, DELIMITER ',');
