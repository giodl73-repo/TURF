...
| Perimeter widened | 5 | `observed_dense` | Body Awareness Studio; Crunch Fitness; LA Fitness; Orangetheory Fitness; Pure Barre |
| Camp Creek | 1 | `observed` | LA Fitness |
| Northlake | 1 | `observed` | LA Fitness |
...
Vinings, Perimeter widened, and North Point / Alpharetta widened are dense
wellness-service fields, while Buckhead, Camp Creek, and Northlake each carry a
checked fitness anchor.

This changes the Atlanta pre-scale read. Cumberland is now finance,
household-service, and wellness dense. Perimeter now adds wellness density to
its transit/health edge-city texture. North Point is open-space, health, and
wellness dense. Camp Creek receives its first observed non-pharmacy context
signal after many earlier source gates, and Northlake moves below the local
source-limited threshold by adding a third observed dimension.
